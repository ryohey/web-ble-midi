...
class MIDIDemo {
  ...
  initializeUI() {
    ...
    if (!BLEMIDI.isSupported()) {
      alert('Your browser does not support Web Bluetooth. Please try using Chrome, Edge, or Opera.');
  ...
  handleMIDIMessage(message) {
    ...
    if (messageType === 0x90 && message.message[2] > 0) {
      // Note On
      this.stats.noteOn++;
    } else if (messageType === 0x80 || (messageType === 0x90 && message.message[2] === 0)) {
      // Note Off
      this.stats.noteOff++;
  ...
  displayMIDIMessage(message) {
    ...
    if (this.displayFormat === 'hex') {
      ...
      if (messageType === 0x90 && message.message[2] > 0) {
        ...
        // SysEx
        dataSpan.textContent = `SysEx: ${Array.from(message.message).map(b => b.toString(16).padStart(2, '0')).join(' ')}`;
        dataSpan.classList.add('message-sysex');
      } else {
        // Other messages
        dataSpan.textContent = `MIDI: ${Array.from(message.message).map(b => b.toString(16).padStart(2, '0')).join(' ')}`;
      }
    }
